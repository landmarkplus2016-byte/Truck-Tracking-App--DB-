/**
 * icons.js — the stroke icon set from the prototype. icon(name) → inline SVG string.
 * Icons draw in currentColor, so CSS decides their colour.
 */
...
export function icon(name, { size, className } = {}) {
  ...
  return `<svg${cls}${dims} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${PATHS[name] || ''}</svg>`;
}
